...
```
# A function retuens current cases for all the country
```{r}
summary<-function(CountryName){
...
findCountryByCode <- function( CountryCode){
         ...
         df<-as.data.frame(data)
  if (CountryCode != "all"){
    df <- filter(df,Countries.CountryCode == CountryCode) %>% select(Countries.Country,everything())
  }
